...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'soner_admin_special'
...
def login():
    ...
    return render_template('login.html') # Ayrı bir login sayfan olduğunu varsayıyorum veya önceki string yöntemi
...
def handle_message(data):
    user = Kullanici.query.get(session.get('user_id'))
    if user:
        # Mesajı odaya göre gönderiyoruz
        emit('yeni_mesaj', {
            'icerik': data['mesaj'], 
